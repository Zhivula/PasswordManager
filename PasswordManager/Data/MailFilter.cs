...
{
    class MailFilter : INotifyPropertyChanged
    {
        ...
        private void Check()
        {
            if (!oldTitle.Equals(Title))
            {
                ...
                {
                    context.Mails.Where(x => x.Id == Id).FirstOrDefault().Title = Title;
                    context.SaveChanges();
                }
            }
            if (!oldPassword.Equals(Password))
            {
                using (var context = new MyDbContext())
                {
                    context.Mails.Where(x => x.Id == Id).FirstOrDefault().Password = RijndaelManagedEncryption.EncryptText(Password, "12345678");
                    context.SaveChanges();
                }
            }
        }
